...
const SettingProfile = () => {
        ...
        if (res.data.data) {
          const fetchedProfile = {
            ...
            resume: {
              ...
              uploadedAt: res.data.data.resume.uploadedAt
                ? new Date(res.data.data.resume.uploadedAt)
                    .toISOString()
                    .slice(0, 16)
                : "",
            },
            coverLetter: {
              ...
              lastUpdated: res.data.data.coverLetter.lastUpdated
                ? new Date(res.data.data.coverLetter.lastUpdated)
                    .toISOString()
                    .slice(0, 16)
                : "",
            },
          ...
          toast.info(
            "No profile found. You can create one from the profile page."
          );
  ...
  const validateForm = () => {
    ...
    if (
      ...
      !/^\+?\d{10,15}$/.test(profile.personalInfo.phone)
    )
      newErrors.phone = "Invalid phone(macron) phone number";
    if (
    ...
    if (
      ...
      !/^https?:\/\/(www\.)?github\.com/.test(profile.personalInfo.githubUrl)
    )
      newErrors.githubUrl = "Invalid GitHub URL";
    if (
      profile.personalInfo.portfolioUrl &&
      !/^https?:\/\/.+/.test(profile.personalInfo.portfolioUrl)
    )
      newErrors.portfolioUrl = "Invalid Portfolio URL";
    if (profile.professionalInfo.yearsOfExperience < 0)
      newErrors.yearsOfExperience = "Years of experience cannot be negative";
    if (profile.professionalInfo.expectedSalary.min < 0)
      newErrors.expectedSalaryMin = "Minimum salary cannot be negative";
    if (
      profile.professionalInfo.expectedSalary.max <
      profile.professionalInfo.expectedSalary.min
    )
      newErrors.expectedSalaryMax =
    ...
    if (
      ...
      !/^https?:\/\/.+/.test(profile.resume.fileUrl)
    )
      newErrors.resumeFileUrl = "Invalid resume URL";
    setErrors(newErrors);
    ...
    if (section === "personalInfo" && subfield === "address") {
      ...
      if (field === "currency") {
        ...
        setProfile((prev) => ({
          ...
          professionalInfo: {
            ...prev.professionalInfo,
            [field]: field === "yearsOfExperience" ? Number(value) || 0 : value,
          },
        }));
      }
      ...
      setProfile((prev) => ({
        ...
        resume: {
          ...prev.resume,
          [field]: field === "fileSize" ? Number(value) || 0 : value,
        },
      }));
  ...
  const handleExperienceChange = (index, field, value) => {
    ...
    updatedExperience[index] = {
      ...updatedExperience[index],
      [field]: field === "isCurrentPosition" ? value : value,
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (profile.professionalInfo.skills.length > 0) {
        const lowerCaseSkills = profile.professionalInfo.skills.map((skill) =>
          skill.toLowerCase()
        );
      ...
      if (profile.professionalInfo.currentTitle) {
        localStorage.setItem(
          "userField",
          profile.professionalInfo.currentTitle.toLowerCase()
        );
      ...
      const payload = {
        ...
        resumeUploadedAt: profile.resume.uploadedAt || new Date().toISOString(),
        resumeFileSize: Number(profile.resume.fileSize),
        coverLetterContent: profile.coverLetter.content,
        coverLetterLastUpdated:
          profile.coverLetter.lastUpdated || new Date().toISOString(),
      };
  ...
  const handleClear = async () => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message ||
        "Failed to clear profile. Please try again.";
      setFetchError(errorMessage);
      toast.error(errorMessage);
    } finally {
      setLoading(false);
        ...
        <>
            ...
            <div className="bg-gradient-to-br from-[#eef2f7] to-[#f9f9fb] min-h-screen py-10 px-4 md:px-10">
                ...
                <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200 sticky top-45 h-fit">
