    ...
    <ul className="space-y-3">
      {items.map((item, idx) => (
        <li key={idx} className="flex items-start gap-3">
          <div
            className={`w-2 h-2 ${bgClassMap[color].split(" ")[0]
              } rounded-full mt-2`}
          ></div>
...
const JobDetails = () => {
  ...
  //  const handleBack = () => {
  //   if (!isAuthenticated) {
  //     navigate("/"); // not logged in, go to home
  //   } else {
  //     if (role === "jobseeker") {
            ...
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`py-3 px-3 rounded-lg font-semibold transition-all ${isAuthenticated && isApplied
                    ? "bg-green-300 text-gray-600 cursor-not-allowed"
                    : "bg-green-200 text-green-700 hover:bg-green-300"
                    }`}
                  disabled={isAuthenticated && isApplied}
                >
                  {isAuthenticated && isApplied ? "Applied" : "Apply Now"}
                </button>
                <button
                  className="p-3 rounded-full bg-white/20 text-gray-700 hover:bg-white/30 transition-all"
                >
                  <Share2 className="w-5 h-5" />
                </button>
                <button
                  onClick={handleSaveJob}
                  className={`p-3 rounded-full transition-all ${isAuthenticated && isSaved
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 sticky top-6">
                <div className="text-center space-y-4">
                  ...
                  <button
                    onClick={() => handleApply(jobDetail._id)}
                    className={`w-full py-3 rounded-lg font-semibold transition-all ${isAuthenticated && isApplied
                      ? "bg-green-300 text-gray-600 cursor-not-allowed"
                  ...
                  <button
                    onClick={handleSaveJob}
                    className={`w-full border border-gray-300 text-gray-700 py-3 rounded-lg font-medium hover:bg-gray-50 transition-all ${isAuthenticated && isSaved ? "bg-yellow-100" : ""
                      }`}
                  >
                    {isAuthenticated && isSaved ? "Saved" : "Save Job"}
                  </button>
                </div>
              </div>
              <InfoCard
